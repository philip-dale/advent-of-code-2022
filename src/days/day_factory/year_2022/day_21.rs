...
impl std::str::FromStr for Shout {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        if s.chars().all(char::is_numeric) {
            ...
            Ok(Self::Ins(Instruction::new(caps[1].to_string(), caps[3].to_string(), &caps[2]).unwrap()))
        }
    }
}
...
impl Shouts {
    ...
    pub fn has_unknown(& self, monkey: &str) -> bool {
        match self.m.get(monkey).unwrap() {
            Shout::Val(_x) => false,
            Shout::Ins(x) => self.has_unknown(x.get_monkeys().0) || self.has_unknown(x.get_monkeys().1),
            Shout::Unknown => true,
        }
    }
    ...
    pub fn set_equal(& mut self, monkey: &str ) {
        let (lhs, rhs) = self.m.get_mut(monkey).unwrap().get_instruction().unwrap().get_monkeys();
        *self.m.get_mut(monkey).unwrap() = Shout::Ins(Instruction::Equ(lhs.to_string(), rhs.to_string()));
    }
    ...
    pub fn set_sub(& mut self, monkey: &str ) {
        let (lhs, rhs) = self.m.get_mut(monkey).unwrap().get_instruction().unwrap().get_monkeys();
        *self.m.get_mut(monkey).unwrap() = Shout::Ins(Instruction::Sub(lhs.to_string(), rhs.to_string()));
    }
